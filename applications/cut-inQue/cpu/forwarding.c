        ...
        if (dst_port < 0) { /* broadcast */
            ...
            for (j = 0; j < app.n_ports; j++) {
                ...
                if (j == i) {
                    ...
                    packet_enqueue(j, worker_mbuf->array[0], tos, fwd_begin);
                } else {
                    new_pkt = rte_pktmbuf_clone(worker_mbuf->array[0], app.pool);
                    packet_enqueue(j, new_pkt, tos, fwd_begin);
                }
            }
        } else {
            RTE_LOG(
            ...
            if (tos <= 0x4)
                ++ app.num_sht[dst_port];
            packet_enqueue(dst_port, worker_mbuf->array[0], tos, fwd_begin);
        }
    }
}
